...
class ABT {
     ...
     */
    duplicateFeature(featureName, newFeatureName = featureName + '-copy', pushToEnd = false) {
        let newFeature = utilities_1.deepCopy(this._features.featureName);
    ...
    keepInstances(condition) {
        let newABT = utilities_1.deepCopy(this);
        newABT.features.forEach(feature => feature.values = []);
        for (let index of utilities_1.range(this.length)) {
 ...
 */
class FinancialABT extends ABT {
    constructor() {
        super();
    }
}
