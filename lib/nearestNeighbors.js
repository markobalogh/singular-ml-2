...
class NearestNeighborsModel extends templateMatchingModel_1.TemplateMatchingModel {
...
class NearestNeighbors extends learningAlgorithm_1.LearningAlgorithm {
    ...
    constructor(config = {}) {
        super();
        //handle different parameter initializations based on the behaviors specified in the constructor.
        if (config.distanceWeighting) {
            this.distanceWeighting = config.distanceWeighting;
        }
        else {
            this.distanceWeighting = distanceWeighting_1.GeneralizedGaussianDistanceWeighting;
        }
        ...
        if (config.zeroDistanceHandling) {
            this.zeroDistanceHandling = config.zeroDistanceHandling;
        }
        else {
            this.zeroDistanceHandling = ZeroDistanceHandling.remove;
        }
        switch (this.distanceWeighting) {
            case distanceWeighting_1.GeneralizedGaussianDistanceWeighting:
                this.sigma = new parameter_1.Parameter(1, false);
            ...
            case distanceWeighting_1.ConstantDistanceWeighting:
                this.k = new parameter_1.Parameter(1, false, 1);
     ...
     */
    setReasonableSigmaBounds(templates) {
        if (templates.length > 1000) {
            let instancesToTest = utilities_1.randomSample(templates, 50);
        }
        else {
            let instancesToTest = utilities_1.randomSample(templates, Math.round(templates.length / 10));
        }
        let lowerBound = Infinity;
        let upperBound = 0;
        let distances = [];
        for (let i = 0; i < templates.length; i++) {
            for (let k = Number(i); k < templates.length; k++) {
                distances.push(this.distanceMetric.evaluate(templates[i], templates[k]));
            }
        }
    ...
    learnFrom(trainingSet) {
        return new NearestNeighborsModel(trainingSet, this.distanceWeighting, this.featureWeighting, this.distanceMetric, this.zeroDistanceHandling, this.normalizations);
    }
}
