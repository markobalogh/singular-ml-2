...
class NearestNeighbors extends templateMatchingModel_1.TemplateMatchingModel {
    constructor(templates, distanceWeighting = distanceWeighting_1.GeneralizedGaussianDistanceWeighting, featureWeighting = false, distanceMetric = distanceMetric_1.EuclideanDistanceMetric, zeroDistanceHandling, normalization) {
        super();
    ...
    vote(distances, queryInstance) {
        ...
        if (distances.includes(0)) {
            if (this.zeroDistanceHandling == ZeroDistanceHandling.continue) {
                ...
                return new prediction_1.Prediction(new instance_1.Instance(votes[distances.findIndex(distance => distance == 0)]));
            }
        ...
        if (weights[0] == 0) {
            ...
            return new prediction_1.Prediction(queryInstance);
        }
    }
    query(instance) {
