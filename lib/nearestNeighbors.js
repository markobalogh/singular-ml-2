...
class NearestNeighborsModel extends templateMatchingModel_1.TemplateMatchingModel {
    ...
    vote(distances, queryInstance) {
        ...
        if (weights[0] == 0) {
            ...
            return new prediction_1.Prediction(votes[0].values.map((value, index) => utilities_1.mean(votes.map(instance => instance.values[index]), weights)), queryInstance.normalizations, votes[0].values.map((value, index) => lodash_1.sum(weights.map(weight => weights[index]))));
        }
    }
    query(instance) {
